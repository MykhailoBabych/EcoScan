...
const RECYCLING_CATEGORIES: CategoryDef[] = [
  {
    category: 'plastic',
    keywords: ['bottle', 'plastic', 'pet', 'water bottle', 'soda', 'polymer', 'polypropylene', 'polyethylene'],
    advice: 'Recycle in plastic bin. Remove cap if required.',
  },
  {
    category: 'metal',
    keywords: ['can', 'aluminum', 'tin', 'beverage can', 'coca-cola', 'coke', 'soda can', 'metal', 'steel'],
  ...
  {
    category: 'food',
    keywords: ['food', 'fruit', 'vegetable', 'bread', 'meat', 'meal', 'snack', 'banana', 'apple', 'organic'],
  ...
  for (const label of labels) {
    ...
    for (const cat of RECYCLING_CATEGORIES) {
      if (cat.keywords.some((kw) => desc.includes(kw))) {
        return { label: label.description, advice: cat.advice, category: cat.category };
      }
    }
  }
...
const styles = StyleSheet.create({
  ...
  camera: { flex: 1 },
  topControls: { flexDirection: 'row', justifyContent: 'flex-end', padding: 20, paddingTop: 50 },
  sideControls: { position: 'absolute', right: 20, top: 120, alignItems: 'center' },
  iconButton: { backgroundColor: 'rgba(0,0,0,0.5)', padding: 10, borderRadius: 8 },
  buttonContainer: {
    flex: 1, flexDirection: 'row', backgroundColor: 'transparent',
    justifyContent: 'center', marginBottom: 100, alignItems: 'flex-end',
  },
  captureButton: {
    backgroundColor: '#28a745', paddingVertical: 15, paddingHorizontal: 30,
    borderRadius: 30, minWidth: 150, alignItems: 'center',
  },
  ...
  resultAdvice: { fontSize: 16, textAlign: 'center', marginBottom: 12, color: '#333' },

  pointsBadge: {
    backgroundColor: '#e8f8ef', borderRadius: 20,
