...
const getEcoAdvice = (labels: any[]) => {
  ...
  for (let label of labels) {
    const desc = label.description.toLowerCase();
    if (
      !ignoredWords.some((iw) => desc.includes(iw)) &&
      !ignoredWords.includes(desc)
    ) {
      return {
        label: label.description,
        advice:
          "We couldn't classify this clearly. Please check your local recycling guidelines.",
      };
    }
  }
  ...
  return {
    ...
    advice:
      "Item not recognized as a standard recyclable. When in doubt, throw it out!",
  };
};
  ...
  const analyzeImage = async (base64: string) => {
    try {
      ...
      if (data.error) {
        ...
        Alert.alert(
          "API Error",
          data.error.message || "Something went wrong with the Vision API.",
        );
        return null;
      }
