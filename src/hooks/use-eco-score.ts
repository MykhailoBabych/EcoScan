...
const RECYCLABLE: WasteCategory[] = [
  'plastic', 'glass', 'paper', 'cardboard', 'metal',
  'textile', 'batteries', 'composite', 'wood', 'toys', 'kitchenware',
];
...
export function useEcoScore() {
  ...
  const recycledCount = RECYCLABLE.reduce((sum, cat) => sum + (categoryStats[cat] ?? 0), 0);
  const wastedCount = POLLUTING.reduce((sum, cat) => sum + (categoryStats[cat] ?? 0), 0);
